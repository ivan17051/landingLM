        ...
        <div class="col-lg-12 aboutgrid2">
          ...
          <div class="card"  style="margin-top:10px;">
            ...
            <table class="table table-bordered">
              <tr>
                <td>
                  ...
                  <h5>{{$tiket->penayangan->nama}}</h5>
                  <h5>{{date("l j F Y H:i",strtotime($tiket->penayangan->tanggal))}}</h5>
                  <h5>{{$tiket->penayangan->deskripsi}}</h5>
                    <h5>{{$tiket->dibeli.' X Rp. '.number_format($tiket->harga)}}</h5>
                </td>
                <td><h5>Rp. {{number_format($tiket->dibeli * $tiket->harga)}}</h5></td>
              </tr>
              <tr><td><h5>Donasi Untuk Tim LEADME</h5></td><td><h5>Rp. {{number_format($tr->donasi)}}</h5></td></tr>
            ...
            <br>
             @if($tr->status=='BELUM DIBAYAR')
            @if(1==2)
            <button onclick="bayars('Rp. {{number_format($tr->total)}}','{{$tr->idtransaksi}}');" class="btn btn-success" style="min-height:50px; width: 100%; margin-bottom:5px;">Bayar Pesanan (Transfer Manual)</button> @endif
            <button class="btn btn-success" style="min-height:50px; width: 100%;"
               data-toggle="modal" data-target="#bayars{{$tr->idtransaksi}}" >Bayar Pesanan (Pengecekan Otomatis)</button>

           
            <div class="modal fade" id="bayars{{$tr->idtransaksi}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
              <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                  ...
                  <div class="modal-body text-center" id="obpay" style="height: 800px;">
                      <object data="https://partner.oyindonesia.com/{{$tr->paymentLinkId}}" width="100%" height="95%"></object>
                  </div>
                </div>
              </div>
            </div>
